...
#'   the following columns: `geo_value` (lowercase abbreviations), and
#'   `location` (fips codes). Defaults to NULL.
#' @param due_date a string specifying the as of date for which to query
#'   the data
#'
...
load_format_daily_data <- function(disease, 
                                   locations_df = NULL, 
                                   due_date = NULL) {
  ...
  daily_data <- covidData::load_data(
    ...
    measure = ifelse(disease == "flu",
                     "flu hospitalizations", "hospitalizations"),
    drop_last_date = FALSE # changed from TRUE
  ) |>
    dplyr::left_join(covidData::fips_codes, by = "location") |>
    dplyr::transmute(
      date=date + lubridate::days(1), # for proper alignment
      location,
      location_name = ifelse(location_name == "United States",
                             "US", location_name),
      value = inc
    ) |>
